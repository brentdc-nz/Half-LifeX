...
#ifndef _XBOX //MARTY - Better diasabled when using a gamepad
	...
	else
	{
		...
		else
		{
			...
			else if( sl->generic.flags & QMF_PULSEIFFOCUS )
			{
				...
				color = PackAlpha( sl->generic.color, 255 * (0.5 + 0.5 * sin( (float)uiStatic.realTime / UI_PULSE_DIVISOR ))); //MARTY
...
void UI_SpinControl_Draw( menuSpinControl_s *sc )
{
	...
	else if( sc->generic.flags & QMF_PULSEIFFOCUS )
	{
		...
		color = PackAlpha( sc->generic.color, 255 * (0.5 + 0.5 * sin( (float)uiStatic.realTime / UI_PULSE_DIVISOR )));//MARTY
...
void UI_Field_Draw( menuField_s *f )
{
	...
	else if( f->generic.flags & QMF_PULSEIFFOCUS )
	{
		...
		color = PackAlpha( f->generic.color, 255 * (0.5 + 0.5 * sin( (float)uiStatic.realTime / UI_PULSE_DIVISOR )));//MARTY
...
void UI_Action_Draw( menuAction_s *a )
{
	...
	else if( a->generic.flags & QMF_PULSEIFFOCUS )
	{
		...
		color = PackAlpha( a->generic.color, 255 * (0.5 + 0.5 * sin( (float)uiStatic.realTime / UI_PULSE_DIVISOR )));//MARTY
...
void UI_Bitmap_Draw( menuBitmap_s *b )
{
	...
	else if( b->generic.flags & QMF_PULSEIFFOCUS )
	{
		...
		color = PackAlpha( b->generic.color, 255 * (0.5 + 0.5 * sin( (float)uiStatic.realTime / UI_PULSE_DIVISOR )));//MARTY
...
void UI_PicButton_Draw( menuPicButton_s *item )
{
	...
	else
	{
		...
		else if( item->generic.flags & QMF_PULSEIFFOCUS )
		{
			...
			color = PackAlpha( item->generic.color, 255 * (0.5 + 0.5 * sin( (float)uiStatic.realTime / UI_PULSE_DIVISOR )));//MARTY
