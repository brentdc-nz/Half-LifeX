...
#ifdef _HARDLINKED //MARTY  - Use Static Linking
...
// MARTY - WIP Some are still missing
...
qboolean SV_CheckClientVisiblity( sv_client_t *cl, const byte *mask )
{
	...
	{
		edict_t	*view = cl->cameras[i];

		if( !SV_IsValidEdict( view ))
			continue;

		leafnum = Mod_PointLeafnum( view->v.origin ) - 1;
...
{
#ifdef _XBOX //MARTY - Unused, done in the server lib on XBox
	return (dword)NULL; //Shutup compiler
#else
...
#ifndef _HARDLINKED //MARTY FIXME WIP - Ok for now. GetNewDLLFunctions does not exist in stock hl.dll
	if( GiveNewDllFuncs )
	{
