...
void DrawSurfaceDecals( msurface_t *fa )
{
	...
	/*p*/glEnable( GL_POLYGON_OFFSET_FILL );
	...
	if( fa->flags & SURF_TRANSPARENT && glState.stencilEnabled )
	{
		...
		for( p = fa->pdecals; p; p = p->pnext )
		{
			if( p->texture )
			{
				...
				/*p*/glStencilFunc( GL_ALWAYS, 1, 0xFFFFFFFF );
				/*p*/glColorMask( GL_FALSE, GL_FALSE, GL_FALSE, GL_FALSE );
				...
				for( i = 0, v = o; i < numVerts; i++, v += VERTEXSIZE )
				{
					...
					/*p*/glTexCoord2f( v[5], v[6] );
					/*p*/glVertex3fv( v );
				}

				/*p*/glEnd();
				/*p*/glStencilOp( GL_KEEP, GL_KEEP, GL_DECR );
				...
				/*p*/glColorMask( GL_TRUE, GL_TRUE, GL_TRUE, GL_TRUE );
				/*p*/glStencilFunc( GL_EQUAL, 0, 0xFFFFFFFF );
				/*p*/glStencilOp( GL_KEEP, GL_KEEP, GL_KEEP );
			}
		}
	}
	...
	for( p = fa->pdecals; p; p = p->pnext )
	{
		if( p->texture )
		{
			...
			if( glt->flags & TF_HAS_ALPHA )
			{
				...
				else /*p*/glTexEnvi( GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_REPLACE );
				/*p*/glBlendFunc( GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA );
			}
			else
			{
