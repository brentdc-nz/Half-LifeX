...
{
	/*p*/glColor4ubv( colors );	// color for this quad

	/*p*/glVertex2f( rect->left, rect->top );
	/*p*/glVertex2f( rect->left + rect->right, rect->top );
	/*p*/glVertex2f( rect->left + rect->right, rect->top + rect->bottom );
	/*p*/glVertex2f( rect->left, rect->top + rect->bottom );
}
...
void SCR_DrawNetGraph( void )
{
	...
	if( net_graph->value < 3 )
	{
		...
		/*p*/glBegin( GL_QUADS ); // draw all the fills as a long solid sequence of quads for speedup reasons
