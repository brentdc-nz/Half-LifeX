...
void GL_SetupFogColorForSurfaces( void )
{
	...
	/*p*/glFogfv( GL_FOG_COLOR, fogColor );
}
...
static void LM_UploadBlock( qboolean dynamic )
{
	...
	if( dynamic )
	{
		...
		/*p*/glTexSubImage2D( GL_TEXTURE_2D, 0, 0, 0, BLOCK_SIZE, height, GL_RGBA, GL_UNSIGNED_BYTE, gl_lms.lightmap_buffer );
	}
	else
	{
...
#ifdef _USEFAKEGL01 // MARTY - let FGLx know the upcoming texture is a sub-texture
		FGL01_SetSubTexture();
#endif
...
void DrawGLPoly( glpoly_t *p, float xScale, float yScale )
{
	...
	/*p*/glBegin( GL_POLYGON );

	for( i = 0, v = p->verts[0]; i < p->numverts; i++, v += VERTEXSIZE )
	{
		if( hasScale )
			/*p*/glTexCoord2f(( v[3] + sOffset ) * xScale, ( v[4] + tOffset ) * yScale );
		else /*p*/glTexCoord2f( v[3] + sOffset, v[4] + tOffset );
...
void DrawGLPolyChain( glpoly_t *p, float soffset, float toffset )
{
	...
	for( ; p != NULL; p = p->chain )
	{
		...
		{
			if( !dynamic ) /*p*/glTexCoord2f( v[5], v[6] );
			else /*p*/glTexCoord2f( v[5] - soffset, v[6] - toffset );
			/*p*/glVertex3fv( v );
		}
		/*p*/glEnd ();
	}
...
void R_BlendLightmaps( void )
{
	...
	if( !CVAR_TO_BOOL( r_lightmap ))
		/*p*/glEnable( GL_BLEND );
	else /*p*/glDisable( GL_BLEND );

	// lightmapped solid surfaces
	/*p*/glDepthMask( GL_FALSE );
	/*p*/glDepthFunc( GL_EQUAL );

	/*p*/glDisable( GL_ALPHA_TEST );
	/*p*/glBlendFunc( GL_ZERO, GL_SRC_COLOR );
...
void R_RenderDetails( void )
{
	...
	/*p*/glEnable( GL_BLEND );
	/*p*/glBlendFunc( GL_DST_COLOR, GL_SRC_COLOR );
	/*p*/glTexEnvi( GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_DECAL );
	/*p*/glDepthFunc( GL_EQUAL );
...
dynamic:
	...
	if( is_dynamic )
	{
		if(( fa->styles[maps] >= 32 || fa->styles[maps] == 0 || fa->styles[maps] == 20 ) && ( fa->dlightframe != tr.framecount ))
		{
			...
			/*p*/glTexSubImage2D( GL_TEXTURE_2D, 0, fa->light_s, fa->light_t, smax, tmax,
...
void R_DrawAlphaTextureChains( void )
{
	...
	/*p*/glAlphaFunc( GL_GREATER, 0.25f );
...
void R_DrawWaterSurfaces( void )
{
	...
	/*p*/glTexEnvi( GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE );
	/*p*/glColor4f( 1.0f, 1.0f, 1.0f, clgame.movevars.wateralpha );
...
void R_SetRenderMode( cl_entity_t *e )
{
	...
	case kRenderTransColor:
		/*p*/glBlendFunc( GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA );
		/*p*/glColor4ub( e->curstate.rendercolor.r, e->curstate.rendercolor.g, e->curstate.rendercolor.b, e->curstate.renderamt );
		/*p*/glTexEnvf( GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE );
		/*p*/glDisable( GL_TEXTURE_2D );
		/*p*/glEnable( GL_BLEND );
		break;
	case kRenderTransAdd:
		/*p*/glTexEnvf( GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE );
		/*p*/glColor4f( tr.blend, tr.blend, tr.blend, 1.0f );
		/*p*/glBlendFunc( GL_ONE, GL_ONE );
		/*p*/glDepthMask( GL_FALSE );
		/*p*/glEnable( GL_BLEND );
		break;
	case kRenderTransAlpha:
		...
		{
			/*p*/glBlendFunc( GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA );
			/*p*/glColor4f( 1.0f, 1.0f, 1.0f, tr.blend );
			/*p*/glEnable( GL_BLEND );
		}
		else
		{
...
void R_DrawTriangleOutlines( void )
{
	...
	/*p*/glColor4f( 1.0f, 1.0f, 1.0f, 1.0f );
	/*p*/glPolygonMode( GL_FRONT_AND_BACK, GL_LINE );
