...
static void R_Clear( int bitMask )
{
	...
	if( CL_IsDevOverviewMode( ))
		/*p*/glClearColor( 0.0f, 1.0f, 0.0f, 1.0f ); // green background (Valve rules)
	else /*p*/glClearColor( 0.5f, 0.5f, 0.5f, 1.0f );
...
#ifndef _XBOX // MARTY - Fixes weapons cliping the near plane with FakeGLx
	zNear = 4.0f;
#else
	zNear = 3.0f;
#endif
...
static void R_SetupFrame( void )
{
	...
	glState.isFogEnabled = /*p*/glIsEnabled( GL_FOG );
...
//		/*p*/glClipPlane( GL_CLIP_PLANE0, clip ); // MARTY TODO - Need to implement, seems to work ok without tho
		/*p*/glEnable( GL_CLIP_PLANE0 );
	}
...
static void R_CheckGLFog( void )
{
	...
	if(( !RI.fogEnabled && !RI.fogCustom ) && /*p*/glIsEnabled( GL_FOG ) && VectorIsNull( RI.fogColor ))
	{
		// fill the fog color from GL-state machine
		/*p*/glGetFloatv( GL_FOG_COLOR, RI.fogColor );
		RI.fogSkybox = true;
	}
#endif
}
...
#else
	if( CL_IsQuakeCompatible( ))
		/*p*/glFogi( GL_FOG_MODE, GL_EXP2 );
	else /*p*/glFogi( GL_FOG_MODE, GL_EXP );
#endif
...
void R_DrawEntitiesOnList( void )
{
	...
	/*p*/glDisable( GL_BLEND );	// Trinity Render issues
...
#ifndef _XBOX // Not required with FakeGLx
	// draw buffer stuff
...
void R_RenderFrame( const ref_viewpass_t *rvp )
{
	...
	if( glConfig.max_multisamples > 1 && FBitSet( gl_msaa->flags, FCVAR_CHANGED ))
	{
		if( CVAR_TO_BOOL( gl_msaa ))
			/*p*/glEnable( GL_MULTISAMPLE_ARB );
		else /*p*/glDisable( GL_MULTISAMPLE_ARB );
...
#ifndef _XBOX // MARTY - Not needed as we don't care about extensions on Xbox
...
===============
*/
qboolean R_InitRenderAPI( void ) // MARTY - Not needed as we don't care about extensions on Xbox
{
