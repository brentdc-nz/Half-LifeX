...
void GL_DrawAliasFrame( aliashdr_t *paliashdr )
{
	...
	while( 1 )
	{
		...
		do
		{
			...
			{
				/*p*/glTexCoord2f( ((float *)order)[0], ((float *)order)[1] );
			}
			order += 2;
			...
			/*p*/glColor4f( g_alias.lightcolor[0] * lv_tmp, g_alias.lightcolor[1] * lv_tmp, g_alias.lightcolor[2] * lv_tmp, tr.blend );
...
static void R_AliasDrawLightTrace( cl_entity_t *e )
{
	if( r_drawentities->value == 7 )
	{
		...
		/*p*/glBegin( GL_LINES );
		/*p*/glColor3f( 1, 0.5, 0 );
		/*p*/glVertex3fv( e->origin );
		/*p*/glVertex3fv( g_alias.lightspot );
		/*p*/glEnd();
...
void R_DrawAliasModel( cl_entity_t *e )
{
	...
	{
		/*p*/glTranslatef( m_pAliasHeader->scale_origin[0], -m_pAliasHeader->scale_origin[1], m_pAliasHeader->scale_origin[2] );
		/*p*/glScalef( m_pAliasHeader->scale[0], -m_pAliasHeader->scale[1], m_pAliasHeader->scale[2] );
	}
	else
	{
		/*p*/glTranslatef( m_pAliasHeader->scale_origin[0], m_pAliasHeader->scale_origin[1], m_pAliasHeader->scale_origin[2] );
	...
	if( GL_Support( GL_ARB_MULTITEXTURE ) && m_pAliasHeader->fb_texturenum[skin][anim] )
	{
		...
		/*p*/glTexEnvf( GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_ADD );
	}
	...
	if( r_shadows.value )
	{
		...
		/*p*/glEnable( GL_BLEND );
		/*p*/glColor4f( 0.0f, 0.0f, 0.0f, 0.5f );
