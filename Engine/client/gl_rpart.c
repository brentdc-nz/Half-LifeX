...
void CL_DrawParticles( double frametime )
{
	...
	for( p = cl_active_particles; p; p = p->next )
	{
		if(( p->type != pt_blob ) || ( p->packedColor == 255 ))
		{
			...
			/*p*/glColor4ub( LightToTexGamma( pColor->r ), LightToTexGamma( pColor->g ), LightToTexGamma( pColor->b ), alpha );
...
void CL_DrawTracers( double frametime )
{
	...
	for( p = cl_active_tracers; p; p = p->next )
	{
		...
		if( !CL_CullTracer( p, start, end ))
		{
			...
			/*p*/glColor4ub( pColor->r, pColor->g, pColor->b, p->packedColor );

			/*p*/glBegin( GL_QUADS );
				/*p*/glTexCoord2f( 0.0f, 0.8f );
				/*p*/glVertex3fv( verts[2] );
				/*p*/glTexCoord2f( 1.0f, 0.8f );
