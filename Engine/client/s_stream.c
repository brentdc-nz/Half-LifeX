...
void S_StreamBackgroundTrack( void )
{
	...
	while( ch->s_rawend < soundtime + ch->max_samples )
	{
		...
		else
		{
			...
			{
				FS_FreeStream( s_bgTrack.stream );
				s_bgTrack.stream = FS_OpenStream( va( "media\\%s", s_bgTrack.loopName )); // MARTY - Fixed slashes
...
=================
*/
#ifdef _AVIPLAYBACK //MARTY - Use for valve logo avi, disabled for now
					//        as we dont have a video decoder atm
