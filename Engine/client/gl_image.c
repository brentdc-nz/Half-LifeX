...
void GL_ApplyTextureParams( gl_texture_t *tex )
{
	...
	if( FBitSet( tex->flags, TF_DEPTHMAP ))
	{
		...
		if( FBitSet( tex->flags, TF_LUMINANCE ))
			/*p*/glTexParameteri( tex->target, GL_DEPTH_TEXTURE_MODE_ARB, GL_LUMINANCE );
		else /*p*/glTexParameteri( tex->target, GL_DEPTH_TEXTURE_MODE_ARB, GL_INTENSITY );
	...
	else
	{
		...
		{
			/*p*/glTexParameteri( tex->target, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR );
			/*p*/glTexParameteri( tex->target, GL_TEXTURE_MAG_FILTER, GL_LINEAR );
		}
...
static void GL_SetTextureFormat( gl_texture_t *tex, pixformat_t format, int channelMask )
{
	...
	else
	{
		...
		{
		// FIXME: This is not correct but seems to work :/
		//		  Need to spend some time to figure out why
		//		  case 2 needs to be set to GL_RGBA for things
		//        to work even though it should not set to this
...
static void GL_TextureImageRAW( gl_texture_t *tex, GLint side, GLint level, GLint width, GLint height, GLint depth, GLint type, const void *data )
{
	...
	if( tex->target == GL_TEXTURE_1D )
	{
		if( subImage ) /*p*/glTexSubImage1D( tex->target, level, 0, width, inFormat, dataType, data );
		else /*p*/glTexImage1D( tex->target, level, tex->format, width, 0, inFormat, dataType, data );
	}
...
#endif
	{
		if( subImage ) /*p*/glTexSubImage2D( cubeTarget + side, level, 0, 0, width, height, inFormat, dataType, data );
		else /*p*/glTexImage2D( cubeTarget + side, level, tex->format, width, height, 0, inFormat, dataType, data );
	}
#ifndef _XBOX
	else if( tex->target == GL_TEXTURE_3D || tex->target == GL_TEXTURE_2D_ARRAY_EXT )
	{
		if( subImage ) /*p*/glTexSubImage3D( tex->target, level, 0, 0, 0, width, height, depth, inFormat, dataType, data );
		else /*p*/glTexImage3D( tex->target, level, tex->format, width, height, depth, 0, inFormat, dataType, data );
	}
#endif
	else // 2D or RECT
	{
		if( subImage ) /*p*/glTexSubImage2D( tex->target, level, 0, 0, width, height, inFormat, dataType, data );
		else /*p*/glTexImage2D( tex->target, level, tex->format, width, height, 0, inFormat, dataType, data );
	}
}

#ifndef _XBOX // We don't have any texture compression in our FGL wrapper atm 
...
static void GL_CheckTexImageError( gl_texture_t *tex )
{
	...
	if( CVAR_TO_BOOL( gl_check_errors ) && ( err = /*p*/glGetError()) != GL_NO_ERROR )
...
static void GL_DeleteTexture( gl_texture_t *tex )
{
	...
	if( glw_state.initialized )
		/*p*/glDeleteTextures( 1, &tex->texnum );
...
#ifdef _USEFAKEGL01	// FGLx01 does not like buffer = NULL
