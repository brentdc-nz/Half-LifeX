...
{
#ifndef _XBOX // We don't have this extension on Xbox atm, not really needed anyway
...
#ifndef _XBOX // Not required on Xbox
...
{
#if defined(_USEFAKEGL01) || defined(_USEFAKEGL09)
	/*p*/wglMakeCurrent( NULL, NULL );
	/*p*/wglDeleteContext( NULL );
#else
	if( pwglMakeCurrent )
		/*p*/wglMakeCurrent( NULL, NULL );
...
qboolean GL_SetPixelformat( void )
{
	...
	glConfig.color_bits = 32;
	glConfig.alpha_bits = 8;
	glConfig.depth_bits = 24;
	glConfig.stencil_bits = 8;
#else
...
#ifndef _XBOX // These cases for failed display setup we don't need on Xbox
...
#ifndef _XBOX // We don't want to auto detect on Xbox
...
static void GL_SetDefaults( void )
{
...
#ifndef _XBOX // TODO
		/*p*/glStencilMask( ( GLuint ) ~0 );
#endif
		/*p*/glStencilFunc( GL_EQUAL, 0, ~0 );
#ifndef _XBOX // TODO
		/*p*/glStencilOp( GL_KEEP, GL_INCR, GL_INCR );
#endif
	}
	...
	/*p*/glAlphaFunc( GL_GREATER, DEFAULT_ALPHATEST );
	/*p*/glEnable( GL_TEXTURE_2D );
	/*p*/glShadeModel( GL_SMOOTH );
	/*p*/glFrontFace( GL_CCW );
...
{
#ifndef _XBOX // Who cares about printing this to console on Xbox?
	Con_Printf( "\n" );
...
void GL_InitCommands( void )
{
	...
	vid_mode = Cvar_Get( "vid_mode", 0, FCVAR_RENDERINFO|FCVAR_VIDRESTART, "display resolution mode" );
#ifndef _XBOX
...
void GL_InitExtensions( void )
{
	...
	if( GL_Support( GL_ARB_MULTITEXTURE ))
		/*p*/glGetIntegerv( GL_MAX_TEXTURE_UNITS_ARB, &glConfig.max_texture_units );

	if( glConfig.max_texture_units == 1 )
		GL_SetExtension( GL_ARB_MULTITEXTURE, false );

#ifndef _XBOX // Extensions we don't have on Xbox via FakeGL, all not required to work, might add some to FGLx in future
	// 3d texture support
	...
	if( GL_Support( GL_SHADER_GLSL100_EXT ))
	{
		/*p*/glGetIntegerv( GL_MAX_TEXTURE_COORDS_ARB, &glConfig.max_texture_coords );
		/*p*/glGetIntegerv( GL_MAX_TEXTURE_IMAGE_UNITS_ARB, &glConfig.max_teximage_units );

		// check for hardware skinning
		/*p*/glGetIntegerv( GL_MAX_VERTEX_UNIFORM_COMPONENTS_ARB, &glConfig.max_vertex_uniforms );
		/*p*/glGetIntegerv( GL_MAX_VERTEX_ATTRIBS_ARB, &glConfig.max_vertex_attribs );
	...
	if( GL_Support( GL_TEXTURE_2D_RECT_EXT ))
		/*p*/glGetIntegerv( GL_MAX_RECTANGLE_TEXTURE_SIZE_EXT, &glConfig.max_2d_rectangle_size );
