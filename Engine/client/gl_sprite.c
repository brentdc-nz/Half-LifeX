...
void R_DrawSpriteModel( cl_entity_t *e )
{
	...
	if( oldframe == frame )
	{
		...
		/*p*/glColor4f( color[0], color[1], color[2], tr.blend );
	...
	else
	{
		...
		if( ilerp != 0.0f )
		{
			/*p*/glColor4f( color[0], color[1], color[2], tr.blend * ilerp );
		...
		if( lerp != 0.0f )
		{
			/*p*/glColor4f( color[0], color[1], color[2], tr.blend * lerp );
			GL_Bind( GL_TEXTURE0, frame->gl_texturenum );
			R_DrawSpriteQuad( frame, origin, v_right, v_up, scale );
		}
	}
	...
	if( R_SpriteHasLightmap( e, psprite->texFormat ))
	{
		...
		/*p*/glColor4f( color2[0], color2[1], color2[2], tr.blend );
