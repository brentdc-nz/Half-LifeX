...
#ifndef _XBOX // MARTY - We don't use the debug console on Xbox
...
#ifdef _VGUI // MARTY - VGUI is not needed for stock HL
	// process VGUI
	VGui_RunFrame ();
#endif
...
void CL_Init( void )
{
	...
	if( !CL_LoadProgs( va( "%s\\client.dll", GI->dll_path ))) // MARTY
