...
#ifdef _XBOX
		FS_AddGameDirectory( va( "D:\\%s\\", dir ), flags ); // MARTY
#else
...
void FS_CreateDefaultGameInfo( const char *filename )
{
	...
	Q_strncpy( defGI.game_dll, /*"dlls/hl.dll"*/"dlls\\hl.dll", sizeof( defGI.game_dll )); // MARTY
...
static qboolean FS_ParseLiblistGam( const char *filename, const char *gamedir, gameinfo_t *GameInfo )
{
	...
	Q_strncpy( GameInfo->game_dll, /*"dlls/hl.dll"*/"dlls\\hl.dll", sizeof( GameInfo->game_dll )); // MARTY
...
static qboolean FS_ParseGameInfo( const char *gamedir, gameinfo_t *GameInfo )
{
	...
	Q_snprintf( default_gameinfo_path, sizeof( default_gameinfo_path ), "%s\\gameinfo.txt", fs_basedir ); // MARTY
	Q_snprintf( gameinfo_path, sizeof( gameinfo_path ), "%s\\gameinfo.txt", gamedir ); // MARTY
	Q_snprintf( liblist_path, sizeof( liblist_path ), "%s\\liblist.gam", gamedir ); // MARTY
...
void FS_Init( void )
{
	...
	stringlistinit( &dirs );
#ifdef _XBOX
	listdirectory( &dirs, /*"./"*/ "D:\\", true ); // MARTY
#else
...
#if 1 // HACK: Very nasty hack fixme
	strTmp = strstr(filepath, "gameinfo.txt");
	if(strTmp)
		file->handle = open( "D:\\valve\\gameinfo.txt", mod|opt, 0666 );
	else
#endif	
...
qboolean FS_SysFolderExists( const char *path )
{
	DWORD	dwFlags;
#ifdef _XBOX
	char tmpStr[MAX_SYSPATH];
	strcpy(tmpStr, "D:\\");
	strcat(tmpStr, path );
#endif
	dwFlags = GetFileAttributes( tmpStr );
...
#ifdef _XBOX
	if( fs_ext_path && ( pEnvPath = "D:\\valve\\"/*getenv( "Path" )*/))
#else
...
//		Q_strcat( search->filename, "\\" ); // MARTY
...
file_t *FS_Open( const char *filepath, const char *mode, qboolean gamedironly )
{
	...
	if( mode[0] == 'w' || mode[0] == 'a'|| mode[0] == 'e' || Q_strchr( mode, '+' ))
	{
		...
		Q_sprintf( real_path, "%s\\%s", fs_writedir, filepath ); // MARTY - Fixed Slashes
