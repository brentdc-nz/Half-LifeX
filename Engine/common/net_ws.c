...
#ifndef _XBOX_WINSOCK // Need on Xbox?
...
#else
	if( ioctlsocket( net_socket, FIONBIO, &optval ) == SOCKET_ERROR )
#endif
	{
...
#else
	if( setsockopt( net_socket, SOL_SOCKET, SO_BROADCAST, (const char *)&optval, sizeof( optval )) == SOCKET_ERROR )
#endif
	{
...
#else
		if( setsockopt( net_socket, SOL_SOCKET, SO_REUSEADDR, (const char *)&optval, sizeof( optval )) == SOCKET_ERROR )
#endif
		{
...
#else // FIXME
//		if( setsockopt( net_socket, IPPROTO_IP, IP_TOS, (const char *)&optval, sizeof( optval )) == SOCKET_ERROR )
#endif
		{
#ifndef _XBOX_WINSOCK
			err = pWSAGetLastError();
...
#else // FIXME
//		if( setsockopt( net_socket, IPPROTO_IP, IP_MULTICAST_LOOP, (const char *)&optval, sizeof( optval )) == SOCKET_ERROR )
#endif
...
void NET_GetLocalAddress( void )
{
	...
	if( net.allow_ip )
	{
		...
		{
#ifndef _XBOX_WINSOCK // FIXME - How to get the IP of the xbox as string?
...
#else
			if( getsockname( net.ip_sockets[NS_SERVER], (struct sockaddr *)&address, &namelen ) == SOCKET_ERROR )
#endif
			{
