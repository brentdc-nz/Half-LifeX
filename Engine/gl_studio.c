...
static void R_StudioDrawPoints( void )
{
	...
	for( j = 0; j < m_pSubModel->nummesh; j++ ) 
	{
		...
		while( i = *( ptricmds++ ))
		{
			...
			if( i < 0 )
			{
				/*p*/glBegin( GL_TRIANGLE_FAN );
				tri_strip = false;
				i = -i;
			}
			else
			{
				/*p*/glBegin( GL_TRIANGLE_STRIP );
				tri_strip = true;
			}
			...
			for( ; i > 0; i--, ptricmds += 4 )
			{
				...
				if(!( g_nForceFaceFlags & STUDIO_NF_CHROME ))
					...
					else if( g_iRenderMode == kRenderTransColor )
					{
						color24	*clr;
						clr = &RI.currententity->curstate.rendercolor;
						/*p*/glColor4ub( clr->r, clr->g, clr->b, alpha * 255 );
					}
					...
					else
					{
						...
						/*p*/glColor4f( lv[0], lv[1], lv[2], alpha );
					}
				}
				...
				if( g_nForceFaceFlags & STUDIO_NF_CHROME )
				{
					...
					/*p*/glVertex3fv( scaled_vertex );
				}
				else
				{
					/*p*/glVertex3f( av[0], av[1], av[2] );
...
static void R_StudioDrawHulls( void )
{
	...
	for( i = 0; i < m_pStudioHeader->numhitboxes; i++ )
	{
		...
		/*p*/glColor4f( hullcolor[j][0], hullcolor[j][1], hullcolor[j][2], alpha );

		/*p*/glBegin( GL_QUAD_STRIP );
		for( j = 0; j < 10; j++ )
			/*p*/glVertex3fv( v2[j & 7] );
		/*p*/glEnd( );
...
static void R_StudioDrawAbsBBox( void )
{
	...
	/*p*/glColor4f( 1.0f, 0.0f, 0.0f, 1.0f );	// red bboxes for studiomodels
	/*p*/glBegin( GL_LINES );
	for( i = 0; i < 2; i += 1 )
	{
		/*p*/glVertex3fv( bbox[i+0] );
		/*p*/glVertex3fv( bbox[i+2] );
		/*p*/glVertex3fv( bbox[i+4] );
		/*p*/glVertex3fv( bbox[i+6] );
		/*p*/glVertex3fv( bbox[i+0] );
		/*p*/glVertex3fv( bbox[i+4] );
		/*p*/glVertex3fv( bbox[i+2] );
		/*p*/glVertex3fv( bbox[i+6] );
		/*p*/glVertex3fv( bbox[i*2+0] );
		/*p*/glVertex3fv( bbox[i*2+1] );
		/*p*/glVertex3fv( bbox[i*2+4] );
		/*p*/glVertex3fv( bbox[i*2+5] );
	}
	/*p*/glEnd();
...
static void R_StudioDrawBones( void )
{
	...
	for( i = 0; i < m_pStudioHeader->numbones; i++ )
	{
		...
		{
//			/*p*/glPointSize( 3.0f ); //MARTY FIXME WIP
...
//			/*p*/glPointSize( 5.0f ); //MARTY FIXME WIP
			/*p*/glColor3f( 0.8f, 0, 0 );
			/*p*/glBegin( GL_POINTS );
			Matrix3x4_OriginFromMatrix( g_bonestransform[i], point );
			/*p*/glVertex3fv( point );
			/*p*/glEnd();
...
//	/*p*/glPointSize( 1.0f ); //MARTY FIXME WIP
	/*p*/glEnable( GL_TEXTURE_2D );
}
...
static void R_StudioSetupRenderer( int rendermode )
{
	...
	/*p*/glShadeModel( GL_SMOOTH );	// enable gouraud shading
...
{
	/*p*/glTexEnvf( GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_REPLACE );
...
===============
*/
void R_StudioDeformShadow( void ) //MARTY FIXME WIP
{
...
static void R_StudioDrawPlanarShadow( void ) //MARTY FIXME WIP
{
	...
	if( r_shadows.value != 0.0f )
	{
		if( RI.currententity->baseline.movetype != MOVETYPE_FLY )
		{
			...
			if( rendermode == kRenderNormal && RI.currententity != &clgame.viewent )
			{
				...
				/*p*/glColor4f( 0.0f, 0.0f, 0.0f, shadow_alpha2 );

				depthmode = GL_LESS;
				/*p*/glDepthFunc( depthmode );

				R_StudioDrawPlanarShadow();

				depthmode2 = GL_LEQUAL;
				/*p*/glDepthFunc( depthmode2 );
