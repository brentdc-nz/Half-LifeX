...
#ifdef _USEFAKEGL01 //MARTY - let FGL know the upcoming texture is a sub-texture
		FGL01_SetSubTexture();
#endif
...
void R_DrawWaterSurfaces( void )
{
	...
	/*p*/glTexEnvi( GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE );
	/*p*/glColor4f( 1.0f, 1.0f, 1.0f, cl.refdef.movevars->wateralpha );
...
void R_DrawBrushModel( cl_entity_t *e )
{
	...
	if( e->curstate.rendermode == kRenderTransTexture && r_lighting_extended->value >= 2.0f )
		/*p*/glBlendFunc( GL_DST_COLOR, GL_ONE_MINUS_SRC_ALPHA );
	...
	case kRenderTransTexture:
		need_sort = true;
	case kRenderGlow:
		/*p*/glColor4ub( 255, 255, 255, e->curstate.renderamt );
		break;
	case kRenderTransColor:
		/*p*/glDisable( GL_TEXTURE_2D );
		/*p*/glColor4ub( e->curstate.rendercolor.r, e->curstate.rendercolor.g,
			e->curstate.rendercolor.b, e->curstate.renderamt );
		break;
	case kRenderTransAlpha:
		// NOTE: brushes can't change renderamt for 'Solid' mode
		/*p*/glAlphaFunc( GL_GEQUAL, 0.5f );
	default:	
		/*p*/glColor4ub( 255, 255, 255, 255 );
		break;
	}
