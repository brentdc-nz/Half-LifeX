...
{
	//MARTY FIXME WIP - Returns NULL, need to setup for static linking
...
void SV_ClearSaveDir( void )
{
	...
	t = FS_Search( "save\\*.HL?", true, true );	// lookup only in gamedir //MARTY - Fixed slashes
...
void SV_AgeSaveList( const char *pName, int count )
{
	...
	Q_snprintf( newName, sizeof( newName ), "save\\%s%02d.sav", pName, count ); //MARTY - Fixed Slashes
	Q_snprintf( newImage, sizeof( newImage ), "save\\%s%02d.bmp", pName, count ); //MARTY - Fixed Slashes
	...
	while( count > 0 )
	{
		...
		{	
			// quick.sav
			Q_snprintf( oldName, sizeof( oldName ), "save\\%s.sav", pName ); //MARTY - Fixed Slashes
			Q_snprintf( oldImage, sizeof( oldImage ), "save\\%s.bmp", pName ); //MARTY - Fixed Slashes
		}
		else
		{	
			// quick04.sav, etc.
			Q_snprintf( oldName, sizeof( oldName ), "save\\%s%02d.sav", pName, count - 1 ); //MARTY - Fixed Slashes
			Q_snprintf( oldImage, sizeof( oldImage ), "save\\%s%02d.bmp", pName, count - 1 ); //MARTY - Fixed Slashes
		}
...
void SV_DirectoryExtract( file_t *pFile, int fileCount )
{
	...
	for( i = 0; i < fileCount; i++ )
	{
		...
		Q_snprintf( szName, sizeof( szName ), "save\\%s", fileName ); //MARTY - Fixed Slashes
...
SAVERESTOREDATA *SV_LoadSaveData( const char *level )
{
	...
	Q_snprintf( name, sizeof( name ), "save\\%s.HL1", level ); //MARTY - Fixed Slashes
...
void SV_EntityPatchWrite( SAVERESTOREDATA *pSaveData, const char *level )
{
	...
	Q_snprintf( name, sizeof( name ), "save\\%s.HL3", level ); //MARTY - Fixed Slashes
...
void SV_SaveClientState( SAVERESTOREDATA *pSaveData, const char *level )
{
	...
	Q_snprintf( name, sizeof( name ), "save\\%s.HL2", level ); //MARTY - Fixed Slashes
...
int SV_SaveGameSlot( const char *pSaveName, const char *pSaveComment )
{
	...
	Q_strncpy( hlPath, "save\\*.HL?", sizeof( hlPath )); //MARTY - Fixed Slashes
	...
	Q_snprintf( name, sizeof( name ), "save\\%s.sav", pSaveName ); //MARTY - Fixed Slashes
...
void SV_SaveGame( const char *pName )
{
	...
	GL_FreeImage( va( "save\\%s.bmp", savename )); //MARTY - Fixed Slashes
...
{
	search_t	*f = FS_Search( "save\\*.sav", true, true );	// lookup only in gamedir //MARTY - Fixed Slashes
	int	i, found = 0;
	long	newest = 0, ft;
	string	savename;	
