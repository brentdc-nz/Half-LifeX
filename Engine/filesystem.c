...
void FS_AddGameHierarchy( const char *dir, int flags )
{
	...
	//MARTY - Modded
	if( dir && *dir ) FS_AddGameDirectory( va( "D:\\valve\\", fs_basedir, dir ), flags );
}
...
static qboolean FS_ParseGameInfo( const char *gamedir, gameinfo_t *GameInfo )
{
	...
	Q_snprintf( filepath, sizeof( filepath ), "%s\\gameinfo.txt"/*"%s/gameinfo.txt"*/, gamedir ); //MARTY
	Q_snprintf( liblist, sizeof( liblist ), "%s\\liblist.gam"/*"%s/liblist.gam"*/, gamedir ); //MARTY
...
file_t *FS_Open( const char *filepath, const char *mode, qboolean gamedironly )
{
	...
	if( mode[0] == 'w' || mode[0] == 'a' || Q_strchr( mode, '+' ))
	{
		...
		Q_sprintf( real_path, "%s\\%s", fs_gamedir, filepath ); //MARTY - Fixed Slashes
