...
#ifdef _XBOX
CVAR_DEFINE_AUTO( sv_autosave, "0", 0, "Use autosave checkpoints" );
#endif
...
void SV_Shutdown( const char *finalmsg )
{
	...
	if( svs.clients )
#if defined(_XBOX) && defined(_HARDLINKED)
		SV_FinalMessage( finalmsg, true );
#else
...
// MARTY: On Xbox we are static linking so if
//        we call SV_UnloadProgs() it screws
//        things up trying to initialize global
//        variables again that should be released
//        when the dll is freed via COM_FreeLibrary()
#if defined(_XBOX) && defined(_HARDLINKED)
	SV_DeactivateServer ();
#else
	NET_Config( false );
