...
static void CL_BulletTracerDraw( particle_t *p, float frametime )
{
	...
	/*p*/glColor4ub( clgame.palette[p->color][0], clgame.palette[p->color][1], clgame.palette[p->color][2], alpha );
	...
	/*p*/glTexCoord2f( 0.0f, fOffset );
	/*p*/glVertex3fv( tmp );

	VectorMA( vecEnd, -width, cross, tmp );
	/*p*/glTexCoord2f( 1.0f, fOffset );
	/*p*/glVertex3fv( tmp );
...
void CL_UpdateParticle( particle_t *p, float ft )
{
	...
	GL_SetRenderMode( kRenderTransTexture );
	/*p*/glColor4ub( color[0], color[1], color[2], alpha );
	...
	/*p*/glVertex3f( p->org[0] - right[0] + up[0], p->org[1] - right[1] + up[1], p->org[2] - right[2] + up[2] );
	/*p*/glTexCoord2f( 0.0f, 0.0f );
	/*p*/glVertex3f( p->org[0] + right[0] + up[0], p->org[1] + right[1] + up[1], p->org[2] + right[2] + up[2] );
	/*p*/glTexCoord2f( 1.0f, 0.0f );
	/*p*/glVertex3f( p->org[0] + right[0] - up[0], p->org[1] + right[1] - up[1], p->org[2] + right[2] - up[2] );
...
void CL_DrawTracer( vec3_t start, vec3_t delta, float width, rgb_t color, int alpha, float startV, float endV )
{
	...
	/*p*/glTexCoord2f( 0.0f, endV );
	/*p*/glVertex3fv( verts[2] );

	/*p*/glTexCoord2f( 1.0f, endV );
	/*p*/glVertex3fv( verts[3] );

	/*p*/glTexCoord2f( 1.0f, startV );
	/*p*/glVertex3fv( verts[1] );

	/*p*/glTexCoord2f( 0.0f, startV );
	/*p*/glVertex3fv( verts[0] );
