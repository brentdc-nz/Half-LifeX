...
const char *Key_KeynumToString( int keynum )
{
	...
	if ( keynum < 0 || keynum > /*255*/KEYCOUNT ) return "<OUT OF RANGE>"; //MARTY
...
void Key_Event( int key, qboolean down )
{
	...
	if( key == K_ESCAPE || key == K_XBOX_START && down ) //MARTY
	{
		switch( cls.key_dest )
		{
		case key_game:
