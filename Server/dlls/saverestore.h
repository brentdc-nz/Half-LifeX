...
public:
	...
	void	WriteFunction( datamap_t *pMap, const char *pname, inputfunc_t **value, int count );		// Save a function pointer
	int		WriteEntVars( const char *pname, entvars_t *pev );		// Save entvars_t (entvars_t)
	int		WriteFields( const char *pname, void *pBaseData, datamap_t *pRootMap, TYPEDESCRIPTION *pFields, int fieldCount );
...
public:
	...
	int		ReadFields( datamap_t *pMap, const char *pname, void *pBaseData, TYPEDESCRIPTION *pFields, int fieldCount );
	int		ReadField( datamap_t *pMap, void *pBaseData, TYPEDESCRIPTION *pFields, int fieldCount, int startField, int size, char *pName, void *pData );
	int		ReadInt( void );
...
#define IMPLEMENT_SAVERESTORE(derivedClass,baseClass) \
	...
	{\
		if ( !baseClass::Save(save) )\
			return 0;\
		return save.WriteFields( #derivedClass, this, GetDataDescMap() , m_SaveData, ARRAYSIZE(m_SaveData) );\
	}\
	int derivedClass::Restore( CRestore &restore )\
	{\
		if ( !baseClass::Restore(restore) )\
			return 0;\
		return restore.ReadFields( GetDataDescMap(), #derivedClass, this, m_SaveData, ARRAYSIZE(m_SaveData) );\
	}
