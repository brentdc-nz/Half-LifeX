...
int CGlobalState::Save( CSave &save )
{
	...
	{
		if ( !save.WriteFields( "GENT", pEntity, NULL, gGlobalEntitySaveData, ARRAYSIZE(gGlobalEntitySaveData) ) )
			return 0;
...
int CGlobalState::Restore( CRestore &restore )
{
	...
	ClearStates();
	if ( !restore.ReadFields( NULL, "GLOBAL", this, m_SaveData, ARRAYSIZE(m_SaveData) ) )
		return 0;
	...
	{
		if ( !restore.ReadFields( NULL, "GENT", &tmpEntity, gGlobalEntitySaveData, ARRAYSIZE(gGlobalEntitySaveData) ) )
			return 0;
