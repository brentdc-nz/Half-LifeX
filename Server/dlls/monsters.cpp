...
int CBaseMonster::Save( CSave &save )
{
	if ( !CBaseToggle::Save(save) )
		return 0;
	return save.WriteFields( "CBaseMonster", this, GetDataDescMap(), m_SaveData, ARRAYSIZE(m_SaveData) );
}
...
int CBaseMonster::Restore( CRestore &restore )
{
	if ( !CBaseToggle::Restore(restore) )
		return 0;
	int status = restore.ReadFields( GetDataDescMap(), "CBaseMonster", this, m_SaveData, ARRAYSIZE(m_SaveData) );
